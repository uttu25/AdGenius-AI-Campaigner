...
import { History, CheckCircle2, XCircle, Calendar, Package, AlertCircle, Clock } from 'lucide-react';
          ...
          <div key={record.id} className={`bg-white border rounded-xl p-6 shadow-sm hover:shadow-md transition-shadow ${isScheduled ? 'border-slate-100 opacity-80' : 'border-slate-200'}`}>
            ...
            <div className="mt-6 pt-4 border-t border-slate-50">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <h4 className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2">Creative Asset Snapshot</h4>
                  <div className="flex gap-4 items-start">
                    ...
                    <p className="text-xs text-slate-600 italic line-clamp-4 leading-relaxed">
                      {record.adCopy ? `"${record.adCopy}"` : "Assets pending generation by Creative Agent."}
                    </p>
                  </div>
                </div>
                ...
                {!isScheduled && record.failureCount > 0 && record.failureReasons && record.failureReasons.length > 0 && (
                  <div className="bg-rose-50/50 p-4 rounded-lg border border-rose-100">
                    <h4 className="text-[10px] font-bold text-rose-600 uppercase tracking-widest mb-2 flex items-center gap-1">
                      <AlertCircle size={12} /> Mission Intelligence Report
                    </h4>
                    ...
                    <p className="mt-3 text-[9px] text-rose-400 italic font-medium uppercase tracking-tighter">
                      Manager Recommendation: Check API Tokens & Phone Validation.
                    </p>
                  </div>
                )}
              </div>
